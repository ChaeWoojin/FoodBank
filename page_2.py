...
prefer_name = st.pills("물품 종류", options, selection_mode="single", label_visibility="collapsed")

if not (prefer_name is None):
    icon = prefer_name[0]
    name = prefer_name[2:]
    prefer_name_code = [k for k, v in preferCnttgClscd.items() if v == name][0]
        ...
        if user_address:
            ...
            if full_addr:
                ...
                if confirm == "예":
                    ...
                    fig.update_traces(marker=dict(color="orange", size=12))
    ...
    with col1:
        st.markdown(f"### {icon} {selected_sido} 내 {name} 보유 센터 지도")
